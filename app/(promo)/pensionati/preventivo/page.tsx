...
//! Specific amounts for pensionati based on the image
const amounts = ['5.000', '10.000', '15.000', '20.000', '25.000', '30.000', '35.000', '40.000', '45.000', '50.000'];
      ...
      <div className="relative group">
        <div className="flex items-center gap-2 mb-1">
          <input
            ...
            className="w-full p-3 md:p-4 rounded-full text-sm md:text-base border border-white/40 bg-white/80 backdrop-blur-sm focus:border-[#40BFEF] focus:ring-2 focus:ring-[#40BFEF]/20 outline-none transition-all group-hover:border-white/60"
          />
          <span className="text-xs text-white/60 absolute right-4 top-1/2 -translate-y-1/2">Opzionale</span>
        </div>
      </div>
      <div className="relative group">
        <input
    ...
    try {
      const emailResponse = await fetch('/api/send-email', {
        ...
        body: JSON.stringify({
          ...
          subject: 'Nuova richiesta di preventivo - Pensionati INPS',
          formData: {
            ...formData,
            amount: `${amount}`,
      ...
      <main className="relative pt-20 pb-16 px-4 sm:px-6 max-w-2xl mx-auto">
        <Card className="p-6 md:p-8 bg-white/5 backdrop-blur-sm rounded-3xl border-white/10">
          ...
          <div className="mb-8 text-center">
            <h1 className="text-2xl md:text-3xl font-bold text-white">Prestito riservato Pensionati INPS</h1>
          </div>
